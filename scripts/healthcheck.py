...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Health check for local predictor runtimes.")
    parser.add_argument("--env", choices=["img", "dl"], required=True)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    result = check_img() if args.env == "img" else check_dl()
