...
from .runtime_utils import open_in_file_manager, suggest_default_dl_runtime
...
class PredictorGUI:
    def __init__(self) -> None:
        ...
        self.deep_runtime_var = tk.StringVar(value=suggest_default_dl_runtime() or DEFAULT_DL_RUNTIME)
    ...
    def _build_sidebar(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(parent, text="Runtime", style="Section.TLabel").grid(row=row, column=0, sticky="w", pady=(24, 0))
        row += 1
        ttk.Label(parent, text="Deep Learning Runtime", style="Hint.TLabel").grid(row=row, column=0, sticky="w", pady=(10, 4))
        row += 1
        ttk.Entry(parent, textvariable=self.deep_runtime_var, style="App.TEntry").grid(row=row, column=0, sticky="ew")
        row += 1
        ttk.Label(
            parent,
            text="Leave the launcher-provided default value, or enter a full Python path if you manage the deep-learning runtime manually.",
            style="Hint.TLabel",
            wraplength=360,
        ).grid(row=row, column=0, sticky="w", pady=(8, 0))
        row += 1
    ...
    def _load_image(self) -> None:
        ...
        self.status_var.set("Image loaded. You can now draw the ROI C circle and deep-learning box.")
    ...
    def _build_request(self) -> PredictionRequest:
        ...
        return PredictionRequest(
            ...
            deep_runtime=self.deep_runtime_var.get().strip(),
        )
